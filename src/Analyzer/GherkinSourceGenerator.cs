...
public class GherkinSourceGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        // 3b. Emit a runtime step catalog writer, keyed only on step metadata and assembly name so
        // editing a .feature file does not retrigger catalog generation.
        var catalogProvider = stepMetadataProvider
            .Combine(context.CompilationProvider.Select((compilation, _) => compilation.AssemblyName ?? "Tests"));

        context.RegisterSourceOutput(catalogProvider, (spc, source) =>
        {
            var (stepMetadata, assemblyName) = source;
            GenerateStepCatalogInitializer(spc, stepMetadata, assemblyName);
        });
    ...
    /// <summary>
    /// Emits a generated module initializer that writes the step catalog to disk, beside the test
    /// assembly, the moment the assembly is loaded (discovery or execution).
    /// </summary>
    /// <param name="context">Source production context to add the generated source to.</param>
    /// <param name="stepMetadata">All step bindings discovered in the compilation.</param>
    /// <param name="assemblyName">Name of the consuming assembly, used to derive the catalog file name.</param>
    private static void GenerateStepCatalogInitializer(SourceProductionContext context, StepMetadataCollection stepMetadata, string assemblyName)
    {
        var catalogMarkdown = StepCatalogBuilder.Build(stepMetadata);
        var escapedMarkdown = catalogMarkdown.Replace("\"", "\"\"");
        var escapedFileName = assemblyName.Replace("\"", "\"\"");

        var source = $$"""
            // <auto-generated/>
            // Writes the step catalog to disk beside the test assembly when this assembly is loaded.
            ...
            namespace Gherkin.Generator.Generated
            {
                internal static class __GherkinStepCatalogInitializer
                {
                    private const string CatalogMarkdown = @"{{escapedMarkdown}}";

                    [System.Runtime.CompilerServices.ModuleInitializer]
                    internal static void Initialize()
                    {
                        global::Gherkin.Generator.Utils.StepCatalogWriter.WriteIfChanged(
                            System.AppContext.BaseDirectory,
                            "{{escapedFileName}}.StepCatalog.md",
                            CatalogMarkdown);
                    }
                }
            }
        ...
        context.AddSource("StepCatalog.g.cs", SourceText.From(source, Encoding.UTF8));
    }
